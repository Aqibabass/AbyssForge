...
export const generateArticle = async (req, res) => {
    try {
        ...
        if (plan !== 'premium' && free_usage >= 10) {
            return res.status(403).json({ success: false, message: 'Free usage limit reached. Please upgrade to premium.' });
        }
...
export const resumeReview = async (req, res) => {
    try {
        ...
        if (resume.size > 5 * 1024 * 1024) {
            return res.status(400).json({ success: false, message: 'Resume file size exceeds the 5MB limit.' });
        ...
        await sql`INSERT INTO creations (user_id, prompt, content, type)
            VALUES (${userId},'Review the uploaded resume', ${content}, 
            'review_resume')`;
